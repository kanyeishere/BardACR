...
public class BardHeartBreakAbility : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (Core.Resolve<JobApi_Bard>().ActiveSong == Song.WanderersMinuet &&
            Core.Resolve<JobApi_Bard>().SongTimer < 45600.0 - wandererSongDuration &&
        ...
        if (Core.Resolve<JobApi_Bard>().ActiveSong == BardUtil.GetSongBySpell(MagesBallad) &&
            Core.Resolve<JobApi_Bard>().SongTimer < 45600.0 - mageSongDuration &&
        ...
        // 旅神期间，不和三层诗心的完美音调冲突
        if (Core.Resolve<JobApi_Bard>().Repertoire == 3 && Core.Resolve<JobApi_Bard>().ActiveSong == Song.WanderersMinuet)
            return -12;
